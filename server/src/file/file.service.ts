...
export class FileService {
  ...
  private validImageTypes = ['png', 'jpeg', 'jpg', 'gif', 'bmp', 'ico', 'jfif', 'pjpeg'];
  ...
  getImageTypeFromData(data: string): string {
    ...
    if (data.search('data:image/') === -1 || !this.validImageTypes.includes(type)) {
  ...
  separateAndStoreImages(html: string): { html: string, filenames: string[] } {
    ...
    try {
      Array.from(images).forEach((elem) => {
        ...
        if (!elem.src) throw new BadRequestException('Image without source received');
        const data = elem.src;
  ...
  joinImages(filenames: string[], html: string): string {
    ...
    Array.from(images).forEach((image) => {
      ...
      let result = src.match(/[\/]?api\/file\/image\/(.*)[\/]?/);
      if (result) {
        // extracted filename
        ...
        if (filenames.includes(filename)) {
          ...
          let data = this.imagePathToBase64(imagesPath, filename);
  ...
  uploadThumbnail(data: string): string {
    ...
    // write the file to the folder - will throw error if unsuccessful
    fs.writeFileSync(
      path.join(thumbnailsPath, filename),
      imageData,
      { encoding: 'base64' },
    );
